...
void setup() {
  ...
  //Serial.begin(31250);      //starts up serial communications. Set for MIDI baud.
  Serial1.begin(31250, SERIAL_8N1, 2, 17);      // Serial1.begin(unsigned long baud, uint32_t config, int8_t rxPin, int8_t txPin, bool invert)
...
void drawFontFaceDemo() {
    ...
    Heltec.display->drawString(0, 10, "MIDI Polyphonic");
    Heltec.display->setFont(ArialMT_Plain_24);
    Heltec.display->drawString(0, 26, "Hello world");
    Serial.println("Font Demo Ran");
}
...
void drawTextFlowDemo() {
    Heltec.display->setFont(ArialMT_Plain_10);
    Heltec.display->setTextAlignment(TEXT_ALIGN_LEFT);
    Heltec.display->drawStringMaxWidth(0, 0, 128, "Here we go. Translating MIDI for Paraphonic." );
    Serial.println("Text Flow Demo");
}
...
void drawImageDemo() {
    ...
    Heltec.display->drawXbm(0, 0, Rubbish_width, Rubbish_height, Rubbish_bits);
}
